 ...
 */
export function serveStatic(_root: string, _options: {
  index?: string;
  dotfiles?: 'allow' | 'deny' | 'ignore';
} = {}): Middleware {
  // const { index = 'index.html', dotfiles = 'ignore' } = options;

  return async (req: SwiftRequest, _res: SwiftResponse, next: () => void) => {
...
export function negotiate(): Middleware {
  return (req: SwiftRequest, _res: SwiftResponse, next: () => void) => {
...
export function logger(format: 'dev' | 'combined' = 'dev'): Middleware {
  return (req: SwiftRequest, res: SwiftResponse, next: () => void) => {
    ...
    res.end = function (chunk?: any, encoding?: BufferEncoding, cb?: (() => void)): SwiftResponse {
      const duration = Date.now() - start;
      const userAgent = req.headers['user-agent'];
      ...
      if (chunk !== undefined) {
        if (encoding && cb) {
          return originalEnd(chunk, encoding, cb) as SwiftResponse;
        } else if (encoding) {
          return originalEnd(chunk, encoding as any) as SwiftResponse;
        } else {
          return originalEnd(chunk) as SwiftResponse;
        }
      } else {
        return originalEnd() as SwiftResponse;
      }
    };
...
export function etag(): Middleware {
  return (req: SwiftRequest, res: SwiftResponse, next: () => void) => {
    ...
    res.end = function (...args: any[]): SwiftResponse {
      if (args.length > 0 && args[0] && req.method === 'GET') {
        const content = typeof args[0] === 'string' ? args[0] : args[0]?.toString() || '';
      ...
      // Call original end with all arguments passed through
